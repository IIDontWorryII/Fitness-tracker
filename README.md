...
![Browsing the exercise catalogue](docs/Screenshots/BrowseExercise.png)
...
Both apps ship with a Dockerfile, and there's a `docker-compose.yml` at the repo root that brings up MongoDB, the backend, and the frontend together:
...
To run them separately instead, build and run each image on its own.
